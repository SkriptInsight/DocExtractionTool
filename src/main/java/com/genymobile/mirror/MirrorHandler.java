...
public class MirrorHandler implements InvocationHandler {
    ...
    private Object getField(GetField getField, Method method) {
        try {
            ...
            ExplicitReturnType explicitReturnType = method.getAnnotation(ExplicitReturnType.class);
            java.lang.Class<?> returnType = method.getReturnType();
            try {
                if (explicitReturnType != null) {
                    returnType = java.lang.Class.forName(explicitReturnType.value());
                }
            } catch (Exception ignored) {}

            return wrapper.wrap(returnType, fieldzz.get(this.object));
